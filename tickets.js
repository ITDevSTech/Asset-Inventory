function ticketsApp() {
  ...
  try {
    ...
    let query = this.db.from('tickets_duplicate').select('*').order('id', { ascending: false });
...
filterTickets() {
 ...
 async fetchTickets() {
            try {
                const { data, error } = await this.db.from('tickets_duplicate').select('*').limit(1000);
 ...
 // Getter para sa filtered assets
  ...
  try {
    ...
    const { error: insertErr } = await this.db
      .from('closed_tickets_duplicate')   // lowercase
      .insert([{
        sn: ticketData.sn,
